...
def updateExcel(moduleName):
    file = open("../{}.v".format(moduleName), "rt", encoding="utf-8")
    flag = True
    n = 0
    linelist = []
    portStartLine = 0
    ...
    while flag:
        linelist.append(file.readline().strip().split())
        if linelist[n] == []:
            n += 1
            break
        if linelist[n][0] == "/*====================Ports":
            ...
            flag = False
        n += 1
    df = [["number", "name", "bit", "type", "in/out", "usage", "expalin"]]
    number = 1
    for i in range(portStartLine, portEndLine):
        if linelist[i][0] == "input" or linelist[i][0] == "output":
            ...
            for k in range(len(name)):
                df.append([number, name[k], bit, linelist[i][1], linelist[i][0], usage, explain])
                number += 1
    for i in df:
        print(i)
    ...
    for i in range(varStartLine, varEndLine):
        if linelist[i][0] == "reg" or linelist[i][0] == "wire":
            ...
            if temp[0] == "[":
                ...
                usage, explain = getother(other)
            for k in range(len(name)):
                df.append([number, name[k], bit, linelist[i][0], None, usage, explain])
                number += 1
    for i in df:
        print(i)            
